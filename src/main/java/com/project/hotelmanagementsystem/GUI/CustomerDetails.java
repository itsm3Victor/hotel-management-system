        ...
        btnAdd.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                if(textField_1.getText().equals(""))
                {
                    ...
                    cust.setCustName(textField.getText());
                    cust.setAddr(textField_1.getText());
                    cust.setPhone(textField_2.getText());
                    cust.setType(comboBox.getSelectedItem().toString());
                    customerService.addCustomer("i",cust);
...
//                    cust.addCustomer("i",cust);
        ...
        btnDelete.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                Customer cust = new Customer();
                cust.setCustName(textField.getText());
                customerService.deleteCustomer("d",cust);
...
//            ResultSet rs= st.executeQuery(query);
            int currentIndex = 0;
            while (currentIndex < customerList.size()){
                model.addRow(new Object[] {
                        customerList.get(currentIndex).getId(),
                        customerList.get(currentIndex).getCustName(),
                        customerList.get(currentIndex).getAddr(),
                        customerList.get(currentIndex).getPhone(),
                        customerList.get(currentIndex).getType()
                });
                currentIndex ++;
            }
//            while(rs.next())
